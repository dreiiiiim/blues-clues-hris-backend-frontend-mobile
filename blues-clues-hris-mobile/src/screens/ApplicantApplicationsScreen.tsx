              ...
              <>
                ...
                {active.map((app) => (
                  <ApplicationCard
                    ...
                    onAcceptOffer={(a) => handleAcceptOffer(a, apps)}
                  />
                ))}
...
function ApplicationCard({
  ...
  readonly offerAccepting: string | null;
  readonly onAcceptOffer: (app: Application) => void;
}) {
  ...
  return (
    <Pressable style={({ pressed }) => [styles.card, pressed && styles.cardPressed]} onPress={onPress}>
      ...
      {/* Accept Offer button */}
      {app.status === 'hired' && (!app.offer_deadline || new Date(app.offer_deadline) >= new Date()) && (
        <TouchableOpacity
          style={{
            marginTop: 8,

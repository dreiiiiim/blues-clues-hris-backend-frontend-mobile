/**
 * ManagerPayslipsScreen — mirrors web behavior where /manager/payslips redirects
 * to /employee/payslips (the manager views their own payslips as an employee would).
 * On mobile we render the same payslips UI but rooted in the Manager portal.
 */
...
function PayslipModal({ payslip, onClose }: { payslip: Payslip; onClose: () => void }) {
  const bd = payslip.breakdown;
  return (
    <Modal visible animationType="slide">
      <SafeAreaView style={{ flex: 1, backgroundColor: "#F8FAFC" }}>
        <View style={d.header}>
          <Text style={d.headerTitle}>Payslip Receipt</Text>
          <Pressable onPress={onClose}><Ionicons name="close" size={22} color="#475569" /></Pressable>
        </View>
        <ScrollView contentContainerStyle={d.content}>
          <View style={d.periodBox}>
            ...
            {payslip.period?.payout_date && <Text style={d.periodText}>Payout: <Text style={d.periodValue}>{new Date(payslip.period.payout_date!).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })}</Text></Text>}
          </View>
          <View style={d.section}>
            ...
            {Number(payslip.total_allowances) > 0 && <View style={d.lineRow}><Text style={[d.lineKey, d.muted]}>Total Allowances</Text><Text style={[d.lineVal, d.muted]}>{toPHP(payslip.total_allowances)}</Text></View>}
          ...
          <View style={d.section}>
            ...
            <View style={d.lineRow}><Text style={[d.lineKey, d.muted]}>Statutory Deductions</Text><Text style={[d.lineVal, d.muted]}>{toPHP(payslip.statutory_deductions)}</Text></View>
...
export function ManagerPayslipsScreen({ route, navigation }: any) {
  ...
  if (loading) {
    return (
      <SafeAreaView style={s.container}>
        <View style={s.centered}><ActivityIndicator size="large" color="#1E40AF" /><Text style={s.loadingText}>Loading payslips...</Text></View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={s.container}>
      <View style={s.inner}>
        {!isMobile && <Sidebar role="manager" userName={session?.name ?? ""} email={session?.email ?? ""} activeScreen="Payslips" navigation={navigation} />}
        <View style={s.main}>
          <ScrollView style={s.scroll} contentContainerStyle={[s.scrollContent, isMobile && { paddingBottom: BOTTOM_TAB_HEIGHT + 8 }]} showsVerticalScrollIndicator={false}>
            ...
            <View style={s.statsGrid}>
              {[
                { label: "Payslips", value: String(payslips.length), hint: "Released payroll receipts." },
          ...
          {isMobile && <BottomTabBar role="manager" activeScreen="Payslips" navigation={navigation} session={session} />}
        </View>
      </View>
      {authOpen && (
        <AuthModal
          onCancel={() => { setAuthOpen(false); setPendingId(null); }}
          onVerified={() => {
            setAuthOpen(false);
            if (pendingId) { const found = payslips.find(p => p.payslip_id === pendingId) ?? null; setOpenPayslip(found); setPendingId(null); }
          }}
        />
      )}

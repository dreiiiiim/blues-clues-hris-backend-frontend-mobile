...
function mapApplicationToCandidate(app: any, index: number): RankedCandidate {
  const sfiaScore = Number(app.sfia_matching_percentage ?? app.pre_screening_score ?? 0);
  const fitPct = Math.min(100, Math.max(0, Math.round(sfiaScore)));

  // Map per-skill SFIA scores if present, else use empty pillars
  const skills: any[] = Array.isArray(app.sfia_skill_scores) ? app.sfia_skill_scores : [];
  const pillars: SFIAPillar[] = skills.length > 0
    ? skills.map((s: any) => ({
        name: s.skill_name ?? s.name ?? "Skill",
        demand: Math.min(100, Math.round(Number(s.required_level ?? 0) * 14.28)),
        supply: Math.min(100, Math.round(Number(s.candidate_level ?? 0) * 14.28)),
    ...
    try {
      ...
      if (rankedRes && rankedRes.ok) {
        ...
        if (appsRes.ok) {
          ...
          const raw: any[] = Array.isArray(data) ? data : data?.data ?? [];
          // Sort by sfia_matching_percentage descending
          const sorted = [...raw].sort(
            (a, b) => Number(b.sfia_matching_percentage ?? b.pre_screening_score ?? 0)
                     - Number(a.sfia_matching_percentage ?? a.pre_screening_score ?? 0)
          );

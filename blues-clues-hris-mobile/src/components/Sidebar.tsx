...
export const Sidebar = ({ role, userName, email = "", activeScreen, navigation, session }: Props) => {
  ...
  const sessionData = session ?? { name: safeUserName, role: safeRole, email };

  const switchTo = (target: string) => {
    navigation.dispatch(
      CommonActions.reset({
        index: 0,
        routes: [{ name: target, params: { session: sessionData } }],
      })
    );
  };
  ...
  const goToScreen = (screenName: string) => {
    ...
    const target = SCREEN_MAP[safeRole]?.[screenName];
    if (target) switchTo(target);
  };

  const openPortalSelect = () => {
    navigation.navigate("PortalSelect", {
      session: sessionData,
      availablePortals: sessionData.availablePortals,
      roleSwitchOptions: sessionData.roleSwitchOptions,
    });
  };

  const menu = MENU_CONFIG[safeRole] ?? [];
    ...
    <>
      <View style={styles.sidebar}>
        ...
        <View style={styles.accountSection}>
          ...
          {Array.isArray(sessionData.availablePortals) && sessionData.availablePortals.length > 1 ? (
            <Pressable style={styles.switchPortalRow} onPress={openPortalSelect}>
              <Ionicons name="shuffle-outline" size={17} color="rgba(255,255,255,0.9)" />

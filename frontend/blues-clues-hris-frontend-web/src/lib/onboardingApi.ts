...
export async function adminUploadTrainingVideo(file: File): Promise<{ url: string; file_name: string; file_type: string; file_size: number }> {
  const formData = new FormData();
  formData.append('file', file);
  const res = await fetch(`${API_BASE_URL}/onboarding/system-admin/training-videos/upload`, {
    method: 'POST',
    headers: { ...(getAccessToken() ? { Authorization: `Bearer ${getAccessToken()}` } : {}) },
    body: formData,
  });
...
export async function adminGetTrainingVideos(templateId?: string): Promise<TrainingVideo[]> {
  ...
  const res = await fetch(`${API_BASE_URL}/onboarding/system-admin/training-videos${qs}`, { headers: headers() });
...
export async function adminCreateTrainingVideo(payload: {
  ...
  const res = await fetch(`${API_BASE_URL}/onboarding/system-admin/training-videos`, {
    method: 'POST',
    headers: headers(),
...
export async function adminUpdateTrainingVideo(videoId: string, payload: Partial<{
  ...
  const res = await fetch(`${API_BASE_URL}/onboarding/system-admin/training-videos/${videoId}`, {
    method: 'PATCH',
    headers: headers(),
    body: JSON.stringify(payload),
  });

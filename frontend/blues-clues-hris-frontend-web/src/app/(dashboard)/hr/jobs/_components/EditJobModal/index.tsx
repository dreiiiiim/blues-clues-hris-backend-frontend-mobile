...
function stepSubtitle(step: 1 | 2 | 3): string {
  switch (step) {
    case 1:
      return "Update the details for this position";
    case 2:
      ...
      <div className="mx-4 flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-border bg-card shadow-2xl">
        <div className="shrink-0 px-6 pb-4 pt-5">
          ...
          <div className="flex w-full items-start">
            ...
            <button type="button" onClick={() => wizard.setStep(2)} className="group flex cursor-pointer flex-col items-center gap-1">
              <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all duration-200 ${wizard.step === 2 ? "bg-primary text-primary-foreground ring-2 ring-primary/20 ring-offset-1 ring-offset-card" : wizard.step > 2 ? "bg-emerald-500 text-white" : "bg-muted text-muted-foreground group-hover:bg-muted/70"}`}>
                {wizard.step > 2 ? <Check className="h-3.5 w-3.5" /> : "2"}
              </div>
              <span className={`whitespace-nowrap text-[10px] font-medium leading-none ${wizard.step === 2 ? "text-primary" : "text-muted-foreground"}`}>App Form</span>
            </button>
            <div className="mx-2 mt-[13px] flex-1">
              <div className={`h-px w-full transition-colors duration-300 ${wizard.step > 2 ? "bg-emerald-400/70" : "bg-border"}`} />
            </div>
            <button type="button" onClick={() => wizard.setStep(3)} className="group flex cursor-pointer flex-col items-center gap-1">
              <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all duration-200 ${wizard.step === 3 ? "bg-primary text-primary-foreground ring-2 ring-primary/20 ring-offset-1 ring-offset-card" : "bg-muted text-muted-foreground group-hover:bg-muted/70"}`}>
                3
              </div>
              <span className={`whitespace-nowrap text-[10px] font-medium leading-none ${wizard.step === 3 ? "text-primary" : "text-muted-foreground"}`}>SFIA Skills</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          {wizard.step === 1 && (
            <Step1JobDetails
              ...
              onSaveClose={(e) => wizard.handleSaveDetails(e, true)}
            />
          )}

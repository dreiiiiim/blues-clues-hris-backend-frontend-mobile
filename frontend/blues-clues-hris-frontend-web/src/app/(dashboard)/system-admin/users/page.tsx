...
function AddUserPanel({ roles, departments, onClose, onCreated }: Readonly<{
  ...
  const [form, setForm] = useState({
    ...
    role_ids: [] as string[], department_id: "", start_date: "",
  });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [loading, setLoading] = useState(false);

  const set = (field: string, value: string) => setForm(f => ({ ...f, [field]: value }));
  const setRoleIds = (ids: string[]) => setForm(f => ({ ...f, role_ids: ids }));
  ...
  const validate = () => {
    ...
    if (form.role_ids.length === 0) e.role_ids = "Select at least one role";
    return e;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        username: form.username, email: form.email, role_ids: form.role_ids,
      };
      if (form.department_id) payload.department_id = form.department_id;
      if (form.start_date) payload.start_date = form.start_date;

      const res = await apiFetch<{ user_id: string; employee_id: string; email: string; username: string; company_id: string | null; first_name?: string; last_name?: string; start_date?: string | null }>("/users", {
        method: "POST", body: JSON.stringify(payload),
      });

      onCreated({
        user_id: res.user_id, employee_id: res.employee_id, username: res.username,
        first_name: res.first_name ?? form.first_name, last_name: res.last_name ?? form.last_name, email: res.email,
        company_id: res.company_id ?? null,
        role_id: form.role_ids[0] ?? null, department_id: form.department_id.trim() || null,
        start_date: res.start_date ?? (form.start_date || null), account_status: "Pending",
        ...
        <div className="flex-1 px-6 py-6 space-y-5 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
              Role <span className="normal-case font-normal text-muted-foreground/70">(select one or more)</span>
            </label>
            <RoleCheckboxGroup
              roles={roles}
...
function EditEmployeeModal({
  ...
  const [form, setForm] = useState({
    ...
    role_ids: (employee.role_ids && employee.role_ids.length > 0)
      ? employee.role_ids
    ...
    <>
      {paged.map(e => (
        <tr key={e.user_id} className="hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setViewEmployee(e)}>
          ...
          <td className="px-5 py-4">
            {(() => {
              const ids = e.role_ids && e.role_ids.length > 0 ? e.role_ids : (e.role_id ? [e.role_id] : []);
              const names = ids.map(id => roles.find(r => r.role_id === id)?.role_name).filter(Boolean) as string[];
              if (names.length === 0) return <span className="text-xs text-muted-foreground">—</span>;
              return (
                <span className="text-xs font-semibold text-foreground flex flex-wrap gap-1">
                  {names[0]}
                  {names.length > 1 && (
                    <span title={names.slice(1).join(", ")} className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-primary/10 text-primary cursor-default">
                      +{names.length - 1}
                    </span>
                  )}
                </span>

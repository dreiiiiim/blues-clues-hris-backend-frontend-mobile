        ...
        <main className="flex-1 overflow-y-auto bg-muted/10 p-4 md:p-8">
          <div className="mx-auto max-w-7xl space-y-6 animate-in fade-in duration-500">
            <section className="relative overflow-hidden rounded-[26px] bg-[linear-gradient(135deg,#0f172a_0%,#172554_52%,#134e4a_100%)] px-7 py-8 text-white shadow-sm">
              <div className="absolute -right-16 -top-16 h-72 w-72 rounded-full bg-blue-500/20 blur-3xl" />
              <div className="absolute -bottom-12 right-48 h-48 w-48 rounded-full bg-emerald-500/15 blur-3xl" />

              <div className="relative z-10">
                <span className="mb-4 inline-block rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-white/80">
                  Subscription Monitoring
                </span>

                <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
                  <div>
                    <h1 className="text-2xl font-bold leading-snug md:text-3xl">Renewals Management</h1>
                  ...
                  <div className="flex shrink-0 items-stretch gap-3">
                    {[
                      ...
                      <div
                        key={item.label}
                        className="flex min-w-18 flex-col items-center justify-center rounded-2xl border border-white/15 bg-white/8 px-5 py-3 backdrop-blur-sm"
                      >
                        <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-white/50">{item.label}</p>
              ...
              {[
                ...
                <div key={label} className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                  ...
                  <div className="p-5">
                    ...
                    <p className="mt-0.5 text-xs text-muted-foreground">Based on current subscription records</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
              <div className="flex flex-col gap-4 border-b border-border px-6 py-5 md:flex-row md:items-center md:justify-between">
                <div>
                  <h2 className="text-base font-bold">Company Renewal List</h2>
                  <p className="mt-0.5 text-xs text-muted-foreground">Review renewal status and take quick actions.</p>
                </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="h-10 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 sm:w-64"
                    />
                  </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredRenewals.map((item) => (
                      <tr key={item.company} className={item.status === "Expired" ? "bg-red-50/30" : "hover:bg-muted/20"}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-bold ${statusStyle(item.status)}`}>
                            {item.status}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-end gap-2">
                            <button className="flex h-8 w-8 items-center justify-center rounded-lg border border-border hover:bg-muted" title="Send reminder">
                              <Bell className="h-4 w-4" />
                            </button>
                            <button className="flex h-8 w-8 items-center justify-center rounded-lg border border-border hover:bg-muted" title="Mark renewed">
                            ...
                            <button className="flex h-8 w-8 items-center justify-center rounded-lg border border-border hover:bg-muted" title="View company">

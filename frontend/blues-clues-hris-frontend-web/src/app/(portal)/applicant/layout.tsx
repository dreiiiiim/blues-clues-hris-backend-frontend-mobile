...
import { applicantRefreshApi, applicantLogoutApi, getMyOnboarding, getMyApplications } from "@/lib/authApi";
...
export default function PortalLayout({
  ...
  useLayoutEffect(() => {
    ...
    const init = async () => {
      ...
      ]).then(([session, record, applications]) => {
        const hasAcceptedOffer = Array.isArray(applications)
          && applications.some((a: any) => a?.status === "offer_accepted");
        if (session || record || hasAcceptedOffer) setOnboardingVisible(true);
      });
    };

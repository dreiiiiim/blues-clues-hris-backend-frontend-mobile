...
import {
  ...
  RotateCcw, DollarSign, AlarmClock, Upload,
} from "lucide-react";
import {
    ...
    <div className="flex items-start gap-0 mt-3">
        ...
        return (
          <div key={stage.key} className="flex items-center flex-1 min-w-0">
            <div className="flex flex-col items-center shrink-0">
              ...
              <span
                className={`mt-1 text-[9px] font-bold uppercase tracking-[0.06em] text-center leading-tight max-w-11
      ...
      <div className="flex items-center flex-1 min-w-0">
        ...
        <div className="flex flex-col items-center shrink-0">
          ...
          >
            {status === "hired" ? "Hired" : status === "rejected" ? "Out" : "Result"}
          </span>
        </div>
      </div>
    </div>
...
function ApplicationCard({ app, onView }: { app: MyApplication; onView: (id: string) => void }) {
  ...
  return (
    <div
      className={`bg-card border rounded-2xl shadow-sm overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer ${
        app.status === "hired"
          ...
          <div className="flex items-start gap-3 min-w-0">
            <div
              className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${
                app.status === "hired"
                  ? "bg-green-500/10 text-green-600 border border-green-200/60 dark:border-green-700/40"
                  : app.status === "rejected"
                  ? "bg-red-500/10 text-red-500 border border-red-200/60 dark:border-red-700/40"
...
function DetailModal({ detail, onClose }: { detail: DetailWithJob; onClose: () => void }) {
  ...
  const sorted = [...detail.answers].sort(
    (a, b) => a.application_questions.sort_order - b.application_questions.sort_order
  );
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 animate-in fade-in duration-200 p-4" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
      ...
      >
        <div className="relative overflow-hidden rounded-t-2xl bg-[linear-gradient(135deg,#0f172a_0%,#172554_55%,#134e4a_100%)] px-6 pt-5 pb-0">
          ...
          <div className="relative flex items-start justify-between gap-3 mb-4">
            ...
            <button
              onClick={onClose}
              className="h-7 w-7 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors border border-white/10 shrink-0 mt-0.5 cursor-pointer"
            >
          ...
          <div className="relative flex items-center gap-0">
            <button
              ...
              className={`px-4 py-2.5 text-xs font-semibold border-b-2 transition-all cursor-pointer ${
                tab === "job" ? "border-white text-white" : "border-transparent text-white/45 hover:text-white/75"
              }`}
            >
              Job Details
            </button>
            ...
            <button
              ...
              className={`px-4 py-2.5 text-xs font-semibold border-b-2 transition-all flex items-center gap-1.5 cursor-pointer ${
                tab === "answers" ? "border-white text-white" : "border-transparent text-white/45 hover:text-white/75"
              }`}
            >
              My Answers
              {sorted.length > 0 && (
              ...
              <div>
                ...
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-4">
                  ...
                  {isTerminal(detail.status) && (
                    <div
                      className={`mt-3 flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-semibold ${
                        detail.status === "hired"
                          ? "bg-green-50 border-green-200 text-green-700 dark:bg-green-900/20 dark:border-green-700/40 dark:text-green-300"
                          : "bg-red-50/60 border-red-200/70 text-red-600 dark:bg-red-900/10 dark:border-red-700/30 dark:text-red-400"
                      }`}
                  ...
                  <div key={ans.answer_id} className="rounded-xl border border-border bg-muted/15 overflow-hidden">
                    <div className="flex items-start gap-2.5 px-4 pt-3 pb-2.5">
                      ...
                      <p className="text-xs font-semibold text-foreground leading-snug">
                        {ans.application_questions.question_text}
                      </p>
                    </div>
    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="grid lg:grid-cols-[1.35fr_0.65fr] gap-4">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <h2 className="text-lg font-bold text-foreground">Keep your resume ready</h2>
              <p className="text-sm text-muted-foreground mt-1">
                Upload your latest resume for future applications. This is a UI prototype for applicant resume upload.
              </p>
            </div>
          ...
          <div className="rounded-2xl border border-dashed border-border bg-muted/20 p-5">
            {!resumeFile ? (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <Button onClick={handleChooseResume} className="h-10 px-4 shrink-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className="h-11 w-11 rounded-xl bg-green-500/10 text-green-600 flex items-center justify-center shrink-0">
                    <FileText className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="font-semibold text-foreground">{resumeFile.name}</p>
                    <p className="text-sm text-muted-foreground mt-1">
                      {formatBytes(resumeFile.size)} • Uploaded {resumeFile.uploadedAt}
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-2 shrink-0">
                  <Button variant="outline" onClick={handleChooseResume} className="h-10 px-4">
                    Replace
                  </Button>
                  <Button variant="outline" onClick={handleRemoveResume} className="h-10 px-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <h2 className="text-lg font-bold text-foreground mb-4">Application checklist</h2>
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 flex items-center justify-between">
              ...
              <span className={`text-xs font-bold ${resumeFile ? "text-green-600" : "text-amber-600"}`}>
                {resumeFile ? "DONE" : "PENDING"}
              </span>
            </div>

            <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">Active applications</span>
            ...
            <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-purple-600">{inInterview}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {!loading && total > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            {filterTabs.map((f) => {
              const Icon = f.icon;
              const isActiveTab = filterStatus === f.key;
              ...
              return (
                ...
                >
                  ...
                  {f.label}
                  <span
                    className={`text-[10px] font-bold min-w-4.5 px-1.5 py-0.5 rounded-full text-center ${
                      isActiveTab ? "bg-white/25 text-white" : "bg-muted text-muted-foreground"
                    }`}
                  >
                    {f.count}
                  </span>
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 max-w-xs">
              ...
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-center rounded hover:bg-muted/60 cursor-pointer"
                >

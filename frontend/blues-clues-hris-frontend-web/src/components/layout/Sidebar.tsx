...
import { isHrPathAllowedForRoles, isHrRoleName } from "@/lib/hrRoleAccess";
...
export function Sidebar({
  ...
  // Collect ALL active HR role names from role_switch_options (one user may hold several HR sub-roles)
  const activeHrRoleNames: string[] =
    persona === "hr" && user?.role_switch_options
      ? user.role_switch_options
          .filter((opt) => opt.portal_key === "hr")
          .map((opt) => opt.role_name)
          .filter(isHrRoleName)
      : user?.role_name && isHrRoleName(user.role_name)
        ? [user.role_name]
        : [];

  const visibleSections =
    persona === "hr" && activeHrRoleNames.length > 0
      ? allSections
          .map((section) => ({
            ...section,
            items: section.items.filter((item) => isHrPathAllowedForRoles(activeHrRoleNames, item.href)),
          }))

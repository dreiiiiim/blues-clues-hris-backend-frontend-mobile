...
export class OnboardingService {
  ...
  async approveSession(sessionId: string, hrUserId?: string) {
    ...
    // ── Sync onboarding_submissions ──────────────────────────────────────
    // When the Wizard is approved, mark the linked onboarding_submissions
    // record as approved too so the two systems stay in sync.
    // The link is: session.account_id (applicant_id before conversion) →
    // onboarding_submissions.applicant_id
    try {
      if (accountId) {
        // Pull staging data for the data-gap sync
        const { data: stagingData } = await supabase
          .from('employee_staging')
          .select('first_name, middle_name, last_name, email_address, phone_number, complete_address, date_of_birth, place_of_birth, nationality, civil_status')
          .eq('session_id', sessionId)
          .maybeSingle();
        ...
        // Map staging data into the submission so the data gap is closed
        if (stagingData) {
          if (stagingData.first_name)      submissionPatch.first_name     = stagingData.first_name;
          if (stagingData.last_name)       submissionPatch.last_name      = stagingData.last_name;
          if (stagingData.phone_number)    submissionPatch.phone          = stagingData.phone_number;
          if (stagingData.complete_address) submissionPatch.address       = stagingData.complete_address;
          if (stagingData.date_of_birth)   submissionPatch.date_of_birth  = stagingData.date_of_birth;
          if (stagingData.nationality)     submissionPatch.nationality     = stagingData.nationality;
          if (stagingData.civil_status)    submissionPatch.civil_status    = stagingData.civil_status;
          if (stagingData.email_address)   submissionPatch.email          = stagingData.email_address;
        }

        // Update whichever submission belongs to this applicant
        ...
        if (matchedSub && matchedSub.status !== 'approved') {
          await supabase
            .from('onboarding_submissions')
            .update(submissionPatch)
            .eq('submission_id', matchedSub.submission_id);

          this.logger.log(`[approveSession] Synced onboarding_submissions ${matchedSub.submission_id} → approved`);
        }
      }
    } catch (subSyncErr) {
      // Non-fatal — wizard approval still succeeds even if submission sync fails
      this.logger.error(`[approveSession] Failed to sync onboarding_submissions: ${(subSyncErr as any)?.message}`);
    }
  ...
  async approveOnboardingSubmission(submissionId: string, roleId: string, companyId: string) {
    ...
    const tokenHash = crypto.createHash('sha256').update(rawToken).digest('hex');
    const expiresAt = new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString();
    // Create a wizard onboarding session so the new employee must complete
    // documents, tasks, HR forms and equipment AFTER account creation.
    try {
      await this.createApplicantSession({
        applicantId:  submission.applicant_id,
        jobPostingId: submission.job_posting_id,
        companyId,
      });
    } catch (sessionErr) {
      this.logger.error(`[approveOnboardingSubmission] Failed to create wizard session: ${(sessionErr as any)?.message}`);
      // non-fatal — employee account is still created
    }

...
import { normalizeNamePart } from '../common/person.utils';
...
export class UsersService {
  ...
  private isMissingDatabaseObject(error: { message?: string } | null | undefined, objectName: string) {
  ...
  async getCompanyInfo(companyId: string) {
    const supabase = this.supabaseService.getClient();
    const [{ data, error }, { data: tenantConfig, error: tenantError }] =
      await Promise.all([
        supabase
          .from('company')
  ...
  async findAll(companyId: string) {
    ...
    const roleIds = [...new Set(users.map((user) => user.role_id).filter((roleId): roleId is string => !!roleId))];

    const [lastLoginByUser, inviteExpiryByUser, roles] = await Promise.all([
  ...
  async update(
    ...
    const { data: user, error: findError } = await supabase
      .from('user_profile')
      .select(
        'user_id, email, first_name, last_name, role_id, department_id, start_date, account_status, middle_name, personal_email, date_of_birth, place_of_birth, nationality, civil_status, complete_address, bank_name, bank_account_number, bank_account_name',
      )
      .eq('user_id', id)
      .eq('company_id', companyId)
      .maybeSingle();
    ...
    if (normalizedFirstName !== undefined) {
      ...
      updates.first_name = normalizeNamePart(normalizedFirstName);
    }
    ...
    if (normalizedLastName !== undefined) {
      ...
      updates.last_name = normalizeNamePart(normalizedLastName);
    }
    ...
    if (updates.middle_name !== undefined) {
      updates.middle_name = normalizeNamePart(updates.middle_name);
    }
    ...
    // Resolve human-readable changes beautifully
    ...
    let beforeRoleName = 'None';
    let afterRoleName = 'None';
    if (updates.role_id !== undefined && user.role_id !== updates.role_id) {
      const { data: roleData } = await supabase
        .from('role')
        .select('role_id, role_name')
        .in('role_id', [user.role_id, updates.role_id].filter(Boolean));
      const roleMap = new Map(roleData?.map((r) => [r.role_id, r.role_name]) || []);
      beforeRoleName = roleMap.get(user.role_id) ?? 'None';
      afterRoleName = roleMap.get(updates.role_id) ?? 'None';
    }

    let beforeDeptName = 'None';
    let afterDeptName = 'None';
    if (updates.department_id !== undefined && user.department_id !== updates.department_id) {
      const { data: deptData } = await supabase
        .from('department')
        .select('department_id, name')
        .in('department_id', [user.department_id, updates.department_id].filter(Boolean));
      const deptMap = new Map(deptData?.map((d) => [d.department_id, d.name]) || []);
      beforeDeptName = deptMap.get(user.department_id) ?? 'None';
      afterDeptName = deptMap.get(updates.department_id) ?? 'None';
    }

    const changesArray: string[] = [];
    for (const field of Object.keys(updates)) {
      const before = user[field] !== undefined ? user[field] : null;
      const after = updates[field];

      // Skip if nothing changed
      if (before === after || (before === null && after === '') || (before === '' && after === null)) {
        continue;
      }
      ...
      if (field === 'role_id') {
        changesArray.push(`${label}: "${beforeRoleName}" → "${afterRoleName}"`);
      } else if (field === 'department_id') {
        changesArray.push(`${label}: "${beforeDeptName}" → "${afterDeptName}"`);
      } else {
        changesArray.push(`${label}: "${before ?? 'None'}" → "${after ?? 'None'}"`);
      }
    }

    const changes = changesArray.length > 0 ? changesArray.join(', ') : 'No fields changed';
  ...
  async updateMe(userId: string, body: {
    ...
    avatar_url?: string;
    personal_notes?: string; // GAP-4.3 FIX
  }) {
    if (
    ...
    // GAP-4.3 FIX: personal_notes added to allowed fields
  ...
  async reviewChangeRequest(
    ...
    if (dto.status === 'approved') {
      ...
      if (request.field_type === 'legal_name') {
        ...
        const firstName = normalizeNamePart(changes.first_name);
        const middleName = normalizeNamePart(changes.middle_name);
        const lastName = normalizeNamePart(changes.last_name);
        if (firstName) nameUpdate.first_name = firstName;
        if (middleName !== undefined) nameUpdate.middle_name = middleName ?? '';
        if (lastName) nameUpdate.last_name = lastName;

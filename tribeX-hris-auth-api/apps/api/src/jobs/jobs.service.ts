...
export class JobsService {
  ...
  async updateApplicationStatus(
    ...
    if (app) {
      ...
      // Send email for all pipeline stage transitions
  ...
  async respondToInterview(applicationId: string, applicantId: string, dto: InterviewResponseDto) {
    ...
    // Notify HR of the applicant's response
    ...
    // Confirm acceptance back to the applicant
  ...
  async acceptOffer(applicationId: string, applicantId: string) {
    ...
    if (!acceptCount || acceptCount === 0)
      throw new BadRequestException('Offer is no longer available for acceptance.');
    ...
    if (otherOffers && otherOffers.length > 0) {
      ...
      if (bulkDeclineErr) {
        ...
        // Retry without offer_declined_at in case column not yet migrated
        ...
        if (retryErr) {
          ...
          // Final fallback: update one-by-one so one bad row does not block all rows
          ...
          this.logger.warn(
            `[acceptOffer] Auto-decline updated ${retryCount ?? 0}/${otherIds.length} offers on retry.`,
          );
        }
      } else if ((bulkDeclineCount ?? 0) < otherIds.length) {
        this.logger.warn(
          `[acceptOffer] Auto-decline updated ${bulkDeclineCount ?? 0}/${otherIds.length} offers.`,
        );
      }

      // Fetch profile once before loop
      const { data: profile } = await supabase
        .from('applicant_profile')
        .select('first_name, last_name, email')
        .eq('applicant_id', applicantId)
    ...
    // Safety net: ensure onboarding session exists for the accepted offer.
    // Some hire-time onboarding creation failures are non-fatal by design, so we re-attempt here.
    try {
      const { data: posting } = await supabase
        .from('job_postings')
        .select('company_id')
      ...
      if (posting?.company_id) {
        ...
        this.logger.warn(
          `[acceptOffer] Skipped onboarding session safety-net; missing company_id for job_posting ${app.job_posting_id}`,
        );
      }
    } catch (onbErr: any) {
  ...
  // ─── Cron — Auto-expire stale hired offers (01:00 AM Manila) ─────────────────

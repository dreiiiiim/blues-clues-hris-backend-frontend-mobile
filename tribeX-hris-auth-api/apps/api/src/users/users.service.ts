...
export class UsersService {
  ...
  private async syncUserPortalAssignments(params: {
    ...
    // Upsert via SELECT + INSERT/UPDATE — user_role_assignments has no unique
    // constraint on (user_id, role_id), so ON CONFLICT fails at the DB level.
  ...
  async findAll(companyId: string) {
    ...
    // Build user_id → role_ids[] from active assignments
    ...
    // Collect all unique role IDs (primary + all assigned) for name lookup
    const allRoleIds = [
      ...new Set([
  ...
  async create(
    ...
    for (const extraRoleId of dto.role_ids.slice(1)) {
      ...
      await this.syncUserPortalAssignments({
        ...
        companyId,
        roleId: extraRoleId,
        roleName: (extraRoleRow as any)?.role_name ?? null,
        setPrimary: false,
      });
    }
  ...
  async update(
    ...
    const roleIdsToAssign: string[] | undefined =
      dto.role_ids && dto.role_ids.length > 0 ? dto.role_ids : undefined;

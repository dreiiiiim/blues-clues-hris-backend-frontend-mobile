...
export class OnboardingService {
  ...
  async uploadTrainingVideo(file: Express.Multer.File) {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      throw new BadRequestException('Invalid file type. Allowed: MP4, WebM, OGG, MOV, AVI, MKV.');
    }
    const maxBytes = 100 * 1024 * 1024; // 100 MB
    if (file.size > maxBytes) {
      throw new BadRequestException('Video file too large. Maximum size is 100 MB.');
    }
    ...
    // Always sync bucket settings so an existing bucket with a stale limit gets updated
  ...
  async submitForReview(sessionId: string, applicantId?: string) {
    const supabase = this.supabaseService.getClient();
    ...
    // ── Training video completion gate ──────────────────────────────────────────
    if (applicantId) {
      ...
      if (sessionRow?.template_id) {
        ...
        if (activeVideos && activeVideos.length > 0) {
          const videoIds = activeVideos.map((v: any) => v.video_id);
          ...
          const completedIds = new Set((completedRows ?? []).map((r: any) => r.video_id));
          const incomplete = videoIds.filter((id: string) => !completedIds.has(id));

          if (incomplete.length > 0) {
            throw new BadRequestException(
              `Please finish all required training videos before submitting. ${incomplete.length} video(s) not yet completed.`,
            );
          }
        }
      }
  ...
  async createTrainingVideo(companyId: string, dto: import('./dto/create-video.dto').CreateVideoDto, createdBy: string) {
    const supabase = this.supabaseService.getClient();
    const { data, error } = await supabase
      .from('onboarding_training_videos')
      .insert({
  ...
  async getTrainingVideos(companyId: string, templateId?: string) {
    const supabase = this.supabaseService.getClient();
    let query = supabase
      .from('onboarding_training_videos')
      .select('*')
  ...
  async updateTrainingVideo(videoId: string, companyId: string, dto: import('./dto/update-video.dto').UpdateVideoDto) {
    const supabase = this.supabaseService.getClient();
    const updates: Record<string, unknown> = { updated_at: new Date().toISOString() };
    if (dto.title           !== undefined) updates.title           = dto.title;
    if (dto.description     !== undefined) updates.description     = dto.description;
    if (dto.video_url       !== undefined) updates.video_url       = dto.video_url;
    if (dto.sequence_order  !== undefined) updates.sequence_order  = dto.sequence_order;
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) throw new BadRequestException(error.message);
    if (!data) throw new NotFoundException('Training video not found');
    return data;
  }

  async deleteTrainingVideo(videoId: string, companyId: string) {
    const supabase = this.supabaseService.getClient();
    const { error } = await supabase
      .from('onboarding_training_videos')
      .delete()
  ...
  // ── Applicant video progress (portal) ─────────────────────────────────────────

  async getApplicantTrainingVideos(applicantId: string) {
    const supabase = this.supabaseService.getClient();

    // Resolve the applicant's active onboarding session to get their template
    const { data: session } = await supabase
      .from('onboarding_sessions')
      .select('template_id')
    ...
    if (!session?.template_id) return []; // no session or template assigned yet
  ...
  async saveVideoProgress(applicantId: string, videoId: string, dto: import('./dto/save-video-progress.dto').SaveVideoProgressDto) {
    const supabase = this.supabaseService.getClient();

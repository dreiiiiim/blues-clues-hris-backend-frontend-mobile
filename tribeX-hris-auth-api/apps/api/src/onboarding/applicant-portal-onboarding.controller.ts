...
export class ApplicantPortalOnboardingController {
  ...
  submitForReview(@Param('sessionId') sessionId: string, @Req() req: any) {
  ...
  @Get('training-videos')
  @ApiOperation({ summary: 'List active training videos with the applicant\'s watch progress' })
  getTrainingVideos(@Req() req: any) {
    return this.onboardingService.getApplicantTrainingVideos(req.user.sub_userid);
  }

  @Post('training-videos/:videoId/progress')
  @ApiOperation({ summary: 'Save video watch progress (bookmark + completion flag)' })
  saveVideoProgress(
